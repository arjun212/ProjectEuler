...
double consecutiveProduct( const vector< int > & digits, int index )
{
	double acc = 1 ;
...
int main()
{
	...
	for( int i = 0; i < digits.size() - WINDOW_SIZE; ++i )
	{
		double cp = consecutiveProduct( digits, i ) ;
		if ( cp > largestValSeen )
		{
			largestValSeen = cp ;
		}
	}
